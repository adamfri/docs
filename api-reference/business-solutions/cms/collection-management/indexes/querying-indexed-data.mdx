...
description: "Indexes accelerate filtering and sorting queries of certain types."
icon: "file-lines"
---

Indexes accelerate filtering and sorting queries of certain types. Data queries are carried out by calling [Query Data Items](https://dev.wix.com/api/rest/wix-data/wix-data/data-items/query-data-items). For details of the query language used in data queries, see [API Query Language](https://dev.wix.com/api/rest/getting-started/api-query-language).

## Filtering

Indexes accelerate queries based on equality operators, such as `eq` and `in`. They also accelerate queries based on range operators, such as `gt`, `gte`, `lt`, and `lte` under certain circumstances. Indexes don't accelerate queries based on the case insensitive `startsWith` operator.
...
However, a query of the second kind isn't accelerated by this index, since the index first orders by `artist`. To speed up a query for all paintings ordered only by `dateCompleted`, create a second index with `dateCompleted` as its first (or only) field.

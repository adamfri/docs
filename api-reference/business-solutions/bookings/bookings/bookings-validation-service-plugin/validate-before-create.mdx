---
title: "Validate Before Create"
description: "Validates whether a booking can be created."
icon: "upload"
tag: "Preview"
mode: "wide"
---
...
<Tabs>
  ...
  <Tab title="REST">
    ...
    Validates whether a booking can be created.

    Wix calls this method when a customer initiates a booking, before the
    booking is persisted. Use this to enforce custom business rules, for example,
    membership plan restrictions, external inventory checks, or fraud prevention.

    The request may contain multiple items for bulk operations. Return a result
    for each item. Items missing a result are treated as valid.

    If your service returns an error or times out, Wix blocks the booking
    creation (fail-closed behavior).

    ## Endpoint

    ```http
    POST /v1/validate-before-create
    ```
    ...
    <ResponseField name="results" type="CreateValidationItemResult[]">

      Validation results. Contains a result for each item in the request.
    ...
    <CodeGroup>
      ...
      ```bash Request
      curl -X POST \ 'https://provider.example.com/v1/validate-before-create' \
      -H 'user-agent: Wix' \
            ...
            "result": {
              ...
              "invalidReason": {
                "fieldViolations": [
                  {
                    "field": "booking",
                    "description": "You've reached your monthly booking limit of 4 sessions. Upgrade to Premium for unlimited bookings.",
                    "code": "BOOKING_LIMIT_REACHED"
                  }
                ],
                "message": "Your membership plan doesn't allow additional bookings this month."
              }
            }
          }
        ]
  ...
  <Tab title="SDK">
    ...
    bookingsValidation.validateBeforeCreate(request, metadata): Promise
    ```

    ## Method Parameters

    <ParamField body="request" type="ValidateBeforeCreateRequest">
    ...
    <input type="radio" name="ex-sdk-validate-before-create" id="ex-sdk-validate-before-create-0" className="wix-picker-radio" defaultChecked />
    <input type="radio" name="ex-sdk-validate-before-create" id="ex-sdk-validate-before-create-1" className="wix-picker-radio" />
    <input type="radio" name="ex-sdk-validate-before-create" id="ex-sdk-validate-before-create-2" className="wix-picker-radio" />

    <input type="checkbox" id="ex-sdk-validate-before-create-open" className="wix-picker-toggle" />

    <label htmlFor="ex-sdk-validate-before-create-open" className="wix-picker-button">

    <span className="wix-picker-current"><span className="wix-picker-name">validateBeforeCreate</span></span>
    <span className="wix-picker-current"><span className="wix-picker-name">validateBeforeCreate (self-hosted)</span><span className="wix-picker-desc">Self-hosted SDK calls require you to [create a client](https://dev.wix.com/docs/sdk/articles/work-with-the-sdk/about-the-wix-client)</span></span>
    <span className="wix-picker-current"><span className="wix-picker-name">validateBeforeCreate (with elevated permissions)</span></span>

    </label>

    <div className="wix-picker-options">

    <label htmlFor="ex-sdk-validate-before-create-0"><span className="wix-picker-name">validateBeforeCreate</span></label>
    <label htmlFor="ex-sdk-validate-before-create-1"><span className="wix-picker-name">validateBeforeCreate (self-hosted)</span><span className="wix-picker-desc">Self-hosted SDK calls require you to [create a client](https://dev.wix.com/docs/sdk/articles/work-with-the-sdk/about-the-wix-client)</span></label>
    <label htmlFor="ex-sdk-validate-before-create-2"><span className="wix-picker-name">validateBeforeCreate (with elevated permissions)</span></label>
    ...
    ```javascript validateBeforeCreate
    import { bookingsValidation } from '@wix/bookings/service-plugins';

    async function validateBeforeCreate(request,metadata) {
      const response = await bookingsValidation.validateBeforeCreate(request,metadata);
    };
    ```
    ...
    ```javascript validateBeforeCreate (self-hosted)
    ...
    async function validateBeforeCreate(request,metadata) {
      const response = await myWixClient.bookingsValidation.validateBeforeCreate(request,metadata);
    };
    ```
    ...
    ```javascript validateBeforeCreate (with elevated permissions)
    import { bookingsValidation } from '@wix/bookings/service-plugins';
    import { auth } from '@wix/essentials';

    async function myValidateBeforeCreateMethod(request,metadata) {
      const elevatedValidateBeforeCreate = auth.elevate(bookingsValidation.validateBeforeCreate);
      const response = await elevatedValidateBeforeCreate(request,metadata);
    }
    ```

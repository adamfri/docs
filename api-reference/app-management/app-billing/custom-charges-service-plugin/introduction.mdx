---
title: "Introduction"
description: "The Custom Charges service plugin (formerly SPI) allows you to manage what customers must pay to use your app."
icon: "file-lines"
---

The Custom Charges service plugin (formerly SPI) allows you to manage what customers must pay to use
...
+ Use the JSON editor to update the `baseUri`. Make sure to include only
  ...
  [Get Charge Limit](fallback::https://dev.wix.com/docs/rest/app-management/app-billing/custom-charges-service-plugin/get-charge-limit)
...
+ Before you can implement the Custom Charges service plugin, it's essential to set up
  [Usage-Based Pricing](https://dev.wix.com/docs/build-apps/build-your-app/pricing-plans/usage-based-pricing)
  for your app in the [app dashboard](fallback::https://dev.wix.com/). Note that
  you have to [submit a request](fallback::https://devforum.wix.com/en/contact)
...
+ You can't bill your customers before they
  [upgrade](fallback::https://support.wix.com/en/article/upgrading-an-app-from-the-wix-app-market)
...
+ You receive notifications in case Wix doesn't accept the charges you return
  via the service plugin. Currently, there is no notification if the customer agrees to
...
+ We recommend to use the `instanceId` instead of the `subscriptionId` to track
  ...
  [App Instance API](fallback::https://dev.wix.com/docs/rest/app-management/app-instance/introduction).
+ If we discover that your app has charged customers for usage outside of an
  ...
  [the Wix App Market team](fallback::https://devforum.wix.com/kb/en/contact) immediately if you
  become aware of any overcharging issues so that we can work together to
  resolve the situation.

## Use cases

+ [Support app upgrades](##support-app-upgrades*fallback::https://dev.wix.com/docs/rest/api-reference/app-management/apps/custom-charges-spi/sample-flows#support-app-upgrades)
+ [Bill a customer](##bill-a-customer*fallback::https://dev.wix.com/docs/rest/api-reference/app-management/apps/custom-charges-spi/sample-flows#bill-a-customer)
+ [Get Notified when an app instance is canceled](##get-notified-when-an-app-instance-is-canceled*fallback::https://dev.wix.com/docs/rest/api-reference/app-management/apps/custom-charges-spi/sample-flows#get-notified-when-an-app-instance-is-canceled)

## Terminology

+ __App instance__: Particular installation of an app that let's you identify
  your customer. Learn more about how to retrieve the instance ID from a
  [service plugin request envelope](fallback::https://dev.wix.com/docs/build-apps/develop-your-app/frameworks/self-hosting/supported-extensions/backend-extensions/add-self-hosted-service-plugin-extensions-with-rest#request-envelope).

...
2. Using [Cart Created](https://dev.wix.com/api/rest/wix-ecommerce/cart/cart-created-webhook), listen for an event when a cart is created.
...
1. Pass the cart's ID (`entityId` field in the above webhook's payload) to the [Update Cart](https://dev.wix.com/api/rest/wix-ecommerce/cart/update-cart) method to add a buyer note to the cart:
  ...
  ************/
  ...
  // Sample cartId:
  const _id = "33d99986-63b0-4f0e-a549-33bda4bef756";

  // Buyer note to add:
  const updateOptions = {
    cartInfo: {
  _id: "33d99986-63b0-4f0e-a549-33bda4bef756",
  buyerNote: "Please ship this ASAP."
  },
  };
...
2. Use [Remove Coupon](https://dev.wix.com/api/rest/wix-ecommerce/cart/remove-coupon) to remove a coupon from a cart.
  ...
  export const myRemoveCouponFunction = webMethod(
    Permissions.Anyone,
    async (cartId) => {
      try {
  ...
  // Sample cartId:
  const cartId = "33d99986-63b0-4f0e-a549-33bda4bef756";

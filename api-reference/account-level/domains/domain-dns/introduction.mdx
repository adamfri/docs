---
title: "Introduction"
description: "With the Domain DNS API you can manage DNS zones in Google's Cloud DNS."
icon: "file-lines"
---

With the Domain DNS API you can manage DNS zones in
[Google's Cloud DNS](fallback::https://cloud.google.com/dns). This allows you to connect
...
+ The Domain DNS API is accessible via API keys. You can't access this
  ...
  child account. But the responses to calls remain consistent across API
...
+ You can only [update DNS zones](fallback::https://dev.wix.com/docs/rest/account-level/domains/domain-dns/update-dns-zone)
...
+ [Update DNS records for a registered domain](fallback::https://dev.wix.com/docs/rest/account-level/domains/domain-dns/sample-flows#update-dns-records-for-a-registered-domain)
+ [Connect an external domain by nameservers to a Wix site](fallback::https://dev.wix.com/docs/rest/account-level/domains/domain-dns/sample-flows#connect-an-external-domain-by-nameservers-to-a-wix-site)
...
+ __Domain name__: The domain name, including the
  [top-level domain](fallback::https://en.wikipedia.org/wiki/Top-level_domain) (TLD).
  Examples for TLDs are `com`, `net`, or `org`.
+ __DNS record__: Database record in the
  [Domain Name System](fallback::https://en.wikipedia.org/wiki/Domain_Name_System) that's
  used to map a human-readable URL to an IP address.
+ __DNS zone__: A [DNS zone](fallback::https://en.wikipedia.org/wiki/DNS_zone)
  is an administrative subdivision of the DNS namespace. When you
  [Create a DNS zone](fallback::https://dev.wix.com/docs/rest/account-level/domains/domain-dns/create-dns-zone)
  the values are uploaded to
  [Google's Cloud DNS](fallback::https://cloud.google.com/dns).

...
### A componentType must be valid for its inputType

The 3 values must agree. Wix builds a field's view by dispatching on `componentType` in the field's `inputType`, so a combination that doesn't exist produces a field with no view. For example, `inputType: NUMBER` supports only `NUMBER_INPUT` and `RATING_INPUT`, so pairing it with `TEXT_INPUT` yields nothing to render.

Creation succeeds either way. Wix stores the field and returns it in `formFields`, but the field has no view, so the form has nothing to display for it. Take each pairing from [Field types](fallback::#field-types) rather than assembling your own.
...
A rich content field shows formatted text, images, or links without collecting a value. Its content is a [Ricos rich content](https://dev.wix.com/docs/rest/crm/ricos/introduction) object, and `maxShownParagraphs` optionally collapses longer content behind an expandable section:
...
`required` sits directly on `inputOptions`, alongside `target` and `inputType`. It isn't part of the validation object:
...
A `required` placed inside `<inputType>Options.validation` isn't part of the schema and has no effect. Creation still succeeds, so nothing signals the mistake, and the field stays optional, the same as any field that omits `required`.

`required` marks a whole field. A multi-line address field also chooses which of its subfields a submitter must fill, in `validation.fields`. See [Contact fields](fallback::#contact-fields).
...
For what a choice field submits, see [Choice](fallback::https://dev.wix.com/docs/api-reference/crm/forms/form-submissions/about-submission-values#choice) in About Submission Values.

## Checkbox labels use rich content

Checkbox and subscribe checkbox fields take a [Ricos rich content](https://dev.wix.com/docs/rest/crm/ricos/introduction) object as their label, not a plain string. This lets a label contain links, which consent text often needs:
...
The site's plan also caps how much a form schema can hold: `fieldsLimit` for input fields, `stepsLimit` for steps, `rulesLimit` for conditional rules, and `emailsLimit` for submission email recipients. It caps how many form schemas the site can have too. Creating a form schema that exceeds a cap fails rather than truncating.

Read the current values with Get Restrictions, which returns the caps for the site along with its current form count. `fieldsLimit` counts input fields, so display elements such as rich content and the submit button don't count toward it. An unset cap means no limit.

Don't read these caps from List Forms Providers Configs. That method returns what each provider app declares for all sites, not the limits in force for a site.

Splitting a single logical form across several schemas to stay under the field cap trades a single submission record for several, and consumes more of the site's form allowance.

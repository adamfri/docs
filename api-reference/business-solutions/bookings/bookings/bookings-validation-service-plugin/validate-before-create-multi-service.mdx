---
title: "Validate Before Create Multi Service"
description: "Validates whether a multi-service booking can be created."
icon: "upload"
tag: "Preview"
mode: "wide"
---
...
<Tabs>
  ...
  <Tab title="REST">
    ...
    Validates whether a multi-service booking can be created.

    Wix calls this method when a customer initiates a multi-service booking,
    before the bookings are persisted. Use this to enforce package-level business
    rules that apply across all bookings in the multi-service package.

    If any booking in the package fails validation, the entire operation is
    blocked. If your service returns an error or times out, Wix blocks the
    creation (fail-closed behavior).

    ## Endpoint

    ```http
    POST /v1/validate-before-create-multi-service
    ```
    ...
    <ParamField body="multiServiceBookingType" type="string">
      ...
      Type of multi-service booking being created. The `multiServiceBookingId` doesn't exist yet at create time.
    ...
    <ParamField body="items" type="CreateValidationItem[]">

      Items to validate. Contains all bookings in the multi-service package.

      Constraints: `maxItems 40`.

      <Expandable title="items properties">

        <ParamField body="itemIndex" type="integer">

          Index of this item in the request, starting from `0`. Used to correlate results back to request items.
    ...
    <ResponseField name="singleServiceBookingResults" type="CreateValidationItemResult[]">
      ...
      <Expandable title="singleServiceBookingResults properties">
        ...
        <ResponseField name="itemIndex" type="integer">

          Index of the validated item from the request.
    ...
    <CodeGroup>
      ...
      ```bash Request
      curl -X POST \ 'https://provider.example.com/v1/validate-before-create-multi-service' \
      -H 'user-agent: Wix' \
          ...
          },
          {
            ...
            "result": {
              ...
              "invalidReason": {
                "fieldViolations": [
                  {
                    "field": "booking.bookedEntity.slot.serviceId",
                    "description": "Your membership doesn't include facial treatments. Add the Facial Package to your membership or book this service separately.",
  ...
  <Tab title="SDK">
    ...
    bookingsValidation.validateBeforeCreateMultiService(request, metadata): Promise
    ```

    ## Method Parameters

    <ParamField body="request" type="ValidateBeforeCreateMultiServiceRequest">
    ...
    <input type="radio" name="ex-sdk-validate-before-create-multi-service" id="ex-sdk-validate-before-create-multi-service-0" className="wix-picker-radio" defaultChecked />
    <input type="radio" name="ex-sdk-validate-before-create-multi-service" id="ex-sdk-validate-before-create-multi-service-1" className="wix-picker-radio" />
    <input type="radio" name="ex-sdk-validate-before-create-multi-service" id="ex-sdk-validate-before-create-multi-service-2" className="wix-picker-radio" />

    <input type="checkbox" id="ex-sdk-validate-before-create-multi-service-open" className="wix-picker-toggle" />

    <label htmlFor="ex-sdk-validate-before-create-multi-service-open" className="wix-picker-button">

    <span className="wix-picker-current"><span className="wix-picker-name">validateBeforeCreateMultiService</span></span>
    <span className="wix-picker-current"><span className="wix-picker-name">validateBeforeCreateMultiService (self-hosted)</span><span className="wix-picker-desc">Self-hosted SDK calls require you to [create a client](https://dev.wix.com/docs/sdk/articles/work-with-the-sdk/about-the-wix-client)</span></span>
    <span className="wix-picker-current"><span className="wix-picker-name">validateBeforeCreateMultiService (with elevated permissions)</span></span>

    </label>

    <div className="wix-picker-options">

    <label htmlFor="ex-sdk-validate-before-create-multi-service-0"><span className="wix-picker-name">validateBeforeCreateMultiService</span></label>
    <label htmlFor="ex-sdk-validate-before-create-multi-service-1"><span className="wix-picker-name">validateBeforeCreateMultiService (self-hosted)</span><span className="wix-picker-desc">Self-hosted SDK calls require you to [create a client](https://dev.wix.com/docs/sdk/articles/work-with-the-sdk/about-the-wix-client)</span></label>
    <label htmlFor="ex-sdk-validate-before-create-multi-service-2"><span className="wix-picker-name">validateBeforeCreateMultiService (with elevated permissions)</span></label>
    ...
    ```javascript validateBeforeCreateMultiService
    import { bookingsValidation } from '@wix/bookings/service-plugins';

    async function validateBeforeCreateMultiService(request,metadata) {
      const response = await bookingsValidation.validateBeforeCreateMultiService(request,metadata);
    };
    ```
    ...
    ```javascript validateBeforeCreateMultiService (self-hosted)
    ...
    async function validateBeforeCreateMultiService(request,metadata) {
      const response = await myWixClient.bookingsValidation.validateBeforeCreateMultiService(request,metadata);
    };
    ```
    ...
    ```javascript validateBeforeCreateMultiService (with elevated permissions)
    import { bookingsValidation } from '@wix/bookings/service-plugins';
    import { auth } from '@wix/essentials';

    async function myValidateBeforeCreateMultiServiceMethod(request,metadata) {
      const elevatedValidateBeforeCreateMultiService = auth.elevate(bookingsValidation.validateBeforeCreateMultiService);
      const response = await elevatedValidateBeforeCreateMultiService(request,metadata);
    }
    ```

...
| `tags.publicTags.tagIds` | `$hasSome`, `$hasAll` | Not sortable |
| `tags.privateTags.tagIds` | `$hasSome`, `$hasAll` | Not sortable |

<Note>

Filtering by `tags.privateTags.tagIds` requires the `PRICING_PLANS.PLAN_READ` permission. Without it, private tags are omitted from responses and can't be used in filters.

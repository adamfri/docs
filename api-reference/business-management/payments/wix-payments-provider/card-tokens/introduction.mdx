---
title: "Introduction"
description: "About the Card Tokens API With Card Tokens API, you can securely store one or more payment details per customer in PCI DSS environment."
icon: "file-lines"
---
...
Tokens cannot be stored or used more than once. Create a new `card_token` for each transaction request. Create and pass a card token to [CreateTransaction](https://dev.wix.com/api/rest/wix-payments/transactions/create-transaction) when a credit/debit card is the chosen payment method.
If you are creating a recurring transaction (MIT/CIT) you only need to tokenize the `card_number` and pass it in the [CreateTransaction](https://dev.wix.com/api/rest/wix-payments/transactions/create-transaction) request together with [COF](##wix-payments_transactions_introduction_interaction-types*fallback::https://dev.wix.com/api/rest/wix-payments/transactions/introduction#wix-payments_transactions_introduction_interaction-types) flags.

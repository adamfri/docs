...
- `STRING`: A string. For a [choice field](fallback::#choice) it's the selected option's value. Date and time fields are strings too, formatted according to the field's `validation.format`.
- `NUMBER`: A number.
- `BOOLEAN`: A boolean.
- `ARRAY`: An array of the [selected option values](fallback::#choice).
...
```json
"services_used_21e8": ["Online store", "Consultation"]
```

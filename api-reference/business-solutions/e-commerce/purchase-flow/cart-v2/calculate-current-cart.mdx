---
title: "Calculate Current Cart"
description: "Calculates the customer's current cart based on its current state (line items, discounts, delivery method, etc.) and returns a detailed summary including subto…"
icon: "upload"
mode: "wide"
---
...
<Tabs>
  ...
  <Tab title="REST">
    ...
    ```http
    POST https://www.wixapis.com/ecom/v2/carts/current/calculate
    ```
    ...
    Calculates the full totals of the current cart.
  ...
  <Tab title="SDK">
    ...
    currentCartV2.calculateCurrentCart(options): Promise
    ```

    ## Method Parameters

    <ParamField body="options" type="CalculateCurrentCartOptions">
    ...
    <input type="radio" name="ex-sdk-calculate-current-cart" id="ex-sdk-calculate-current-cart-0" className="wix-picker-radio" defaultChecked />
    <input type="radio" name="ex-sdk-calculate-current-cart" id="ex-sdk-calculate-current-cart-1" className="wix-picker-radio" />

    <input type="checkbox" id="ex-sdk-calculate-current-cart-open" className="wix-picker-toggle" />

    <label htmlFor="ex-sdk-calculate-current-cart-open" className="wix-picker-button">

    <span className="wix-picker-current"><span className="wix-picker-name">Calculate current cart totals</span></span>
    <span className="wix-picker-current"><span className="wix-picker-name">calculateCurrentCart (self-hosted)</span><span className="wix-picker-desc">Self-hosted SDK calls require you to [create a client](https://dev.wix.com/docs/sdk/articles/work-with-the-sdk/about-the-wix-client)</span></span>

    </label>

    <div className="wix-picker-options">

    <label htmlFor="ex-sdk-calculate-current-cart-0"><span className="wix-picker-name">Calculate current cart totals</span></label>
    <label htmlFor="ex-sdk-calculate-current-cart-1"><span className="wix-picker-name">calculateCurrentCart (self-hosted)</span><span className="wix-picker-desc">Self-hosted SDK calls require you to [create a client](https://dev.wix.com/docs/sdk/articles/work-with-the-sdk/about-the-wix-client)</span></label>
    ...
    ```javascript Calculate current cart totals
    import { currentCartV2 } from "@wix/ecom";

    async function calculateCurrentCart() {
      const response = await currentCartV2.calculateCurrentCart({
        refreshCart: true,
      });
    }
    ...
    ```javascript calculateCurrentCart (self-hosted)
    ...
    async function calculateCurrentCart(options) {
      const response = await myWixClient.currentCartV2.calculateCurrentCart(options);
    };
    ```

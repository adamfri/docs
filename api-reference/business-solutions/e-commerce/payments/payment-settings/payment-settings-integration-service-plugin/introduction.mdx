---
title: "Introduction"
description: "As a payment settings service plugin (formerly SPI), you can integrate your service with Wix's payment process to allow merchants to request and use your servi…"
icon: "file-lines"
---
...
The integration is done via an app in the Wix App Market (created in the [app dashboard](fallback::https://dev.wix.com/apps/my-apps?viewId=active-apps-view)) and the Wix Payment Settings SPI.  
...
To collect payments, a Wix site must [connect a payment provider](fallback::https://support.wix.com/en/article/connecting-a-payment-provider). Once a payment provider is connected you may use the Payment Settings service plugin to enforce 
additional payment settings to a site's transactions. For example, a site may require additional [3d secure payments](fallback::https://support.wix.com/en/article/about-3d-secure-3ds-payments-with-third-party-payment-providers) 
(3DS) for certain payment methods.
Note that each payment provider may have specific payment settings they accept with their Wix integration. For example, [Tranzila](fallback::https://support.wix.com/en/article/connecting-tranzila-as-a-payment-provider) 
is a payment provider that supports 3DS payments, but not all payment providers offer this feature. We recommend [contacting payment providers directly](fallback::https://support.wix.com/en/article/receiving-payouts-from-third-party-payment-providers) 
...
1. Go to [Extensions](fallback::https://dev.wix.com/app-selector?title=Select+an+App&primaryButtonText=Select+Site&actionUrl=https%3A%2F%2Fdev.wix.com%2Fapps%2F%7BappId%7D%2Fextensions) in your app's dashboard.

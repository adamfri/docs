...
This article presents sample flows you can support. You aren't limited to this exact flow, but it can be a helpful jumping off point as you plan your catalog integration.

## Add a catalog item to a cart

<Note>

In this example, item variant details are passed in the request. Learn more about this approach to [handling item variants](##approach-1--pass-variant-details-in-options*fallback::https://dev.wix.com/docs/rest/business-solutions/e-commerce/service-plugins/catalog-service-plugin/handle-item-variants#approach-1--pass-variant-details-in-options).

</Note>

In this flow, a site visitor adds an item to their cart, and then Wix calls the Catalog service plugin to retrieve the item's full details. 
...
1. The app code calls [Add To Current Cart](https://dev.wix.com/docs/rest/business-solutions/e-commerce/cart/add-to-current-cart) with the following `lineItems` array in the payload to create a new cart that includes the item:
...
1. Wix automatically calls your app's implementation of [Get Catalog Items](https://dev.wix.com/docs/rest/business-solutions/e-commerce/service-plugins/catalog-service-plugin/get-catalog-items) with the following payload to retrieve the item's full and current details:
...
1. Your implementation of Get Catalog Items returns a response like this one:
    ...
    + The response must include all fields marked as required in the [response description](https://dev.wix.com/docs/rest/business-solutions/e-commerce/service-plugins/catalog-service-plugin/get-catalog-items).
...
In this example, item variant details are processed externally via a custom API that returns a unique ID. Learn more about this approach to [handling item variants](##approach-2--pass-a-variant-id-in-options*fallback::https://dev.wix.com/docs/rest/business-solutions/e-commerce/service-plugins/catalog-service-plugin/handle-item-variants#approach-2--pass-a-variant-id-in-options).

</Note>

In this flow, a site visitor adds an item to their checkout. Wix calls the Catalog service plugin to retrieve the item's full details. 
...
1. Wix automatically calls your app's implementation of [Get Catalog Items](https://dev.wix.com/docs/rest/business-solutions/e-commerce/service-plugins/catalog-service-plugin/get-catalog-items) with the following request object to retrieve the item's full and current details:

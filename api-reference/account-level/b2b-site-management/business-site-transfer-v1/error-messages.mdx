...
description: "This article outlines error messages that might be issued when calling endpoints of the B2B Site Management API."
icon: "file-lines"
---
...
| `PERMISSION_DENIED (403)` | `SITE_TRANSFER_NOT_ALLOWED` | Site transfer isn't allowed for `sourceAccountId` and `targetAccountId`.  | Contact the [Wix B2B Sales team](fallback::mailto:bizdev@wix.com) to check that the specified target and source accounts are supported for site transfers. |
...
| `FAILED_PRECONDITION (428)` | `SITE_TRANSFER_NOT_SUPPORTED` | Site transfer isn't supported for sites with paid Wix services. | The specified site includes a paid Wix service. Use the [Cancel Package endpoint of the Resellers API](https://dev.wix.com/api/rest/account-level-apis/resellers/packages/cancel-package) to remove the paid service before trying to transfer the site again. After the transfer, you can use the [Create Package endpoint of the Resellers API](fallback::https://dev.wix.com/docs/rest/account-level/resellers/packages-and-product-instances/create-package-v2) to add the paid Wix service to the site again. |
